...
- **Small** (typo, rename, config tweak, parameter value change): Implement → Quick Verify (5a + 5b only — no ledger, no adversarial review, no evidence bundle). Exception: 🔴 files escalate to Large (3 reviewers).
...
2. **AVM module version discovered?** → `store_memory` the module path and version if it's not in `copilot-instructions.md`.
3. **Codebase pattern found in existing code (Step 2) not in instructions?** → `store_memory`
4. **Reviewer caught something your verification missed?** → `store_memory` the gap and how to check for it next time.
5. **Fixed a regression you introduced?** → `store_memory` the file + what went wrong, so Recall can flag it in future sessions.
